...
//        product.getParts().addAll(assparts);
    ...
    //    this.product=product;
        ...
        // builds a list of available parts to add to product
        List<Part>availParts=new ArrayList<>();
        for(Part p: partService.findAll()){
...
// make the add and remove buttons work
 ...
 //        return "confirmationassocpart";
    ...
    public String buyProduct(@RequestParam("productID") int theId, Model theModel) {
        ...
        // Creating object called guitar from product class
        Product guitar = productService.findById(theId);

        // Create variable to store value of inventory
        int inv = guitar.getInv();

        // check to see if inv value is zero
        if(inv == 0) {
            ...
            // set new value of guitar inventory
            guitar.setInv(inv);

            // save guitar object with new inventory value
